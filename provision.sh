...
#   %www-data ALL=(ALL:ALL) NOPASSWD:/usr/sbin/service php7.1-fpm restart,/usr/sbin/service nginx restart
#
#--------------------------------------------------------------------------
# How to run
#--------------------------------------------------------------------------
...
apt-get install -y --force-yes --allow-downgrades --allow-remove-essential --allow-change-held-packages \
nginx php7.1-fpm
...
sed -i "s/expose_php = .*/expose_php = Off/" /etc/php/7.1/fpm/php.ini
...
sed -i "s/user = www-data/user = ${USERNAME}/" /etc/php/7.1/fpm/pool.d/www.conf
sed -i "s/group = www-data/group = ${USERNAME}/" /etc/php/7.1/fpm/pool.d/www.conf

sed -i "s/listen\.owner.*/listen.owner = ${USERNAME}/" /etc/php/7.1/fpm/pool.d/www.conf
sed -i "s/listen\.group.*/listen.group = ${USERNAME}/" /etc/php/7.1/fpm/pool.d/www.conf
